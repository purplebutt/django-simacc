...
def generate_cfledger(cash_flow, period_from:date=None, period_to:date=None):
    if not period_from: period_from = timezone.now().date() - timedelta(days=30)
    if not period_to: period_to = timezone.now().date()
    beginning_balance = JRE.objects.filter(cashflow=cash_flow, date__lt=period_from).annotate(
        amount2=Sum(
            Case( When(group='d', then=F('amount')),
                  When(group='c', then=F('amount')*-1), default=0),
        )
    ...
    result = JRE.objects.filter(cashflow=cash_flow, date__gte=period_from, date__lte=period_to).annotate(
        debit=Sum(Case(
            When(group='d', then=F('amount')), default=0,
        )),
        credit=Sum(Case(
